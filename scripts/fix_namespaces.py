...
def fix_hwpx_namespaces(hwpx_path):
    ...
    Raises:
        ValueError: ZIP 엔트리가 안전하지 않거나 크기 제한 초과 시
    """
    NS_MAP = {
    ...
    target_dir = os.path.dirname(os.path.abspath(hwpx_path))
    fd, tmp_path = tempfile.mkstemp(suffix=".hwpx.tmp", dir=target_dir)
    os.close(fd)
            ...
            with zipfile.ZipFile(tmp_path, "w", zipfile.ZIP_DEFLATED) as zout:
                for item in zin.infolist():
                    if not _is_safe_zip_entry(item.filename):
                        raise ValueError(f"Unsafe ZIP entry path: {item.filename}")
                    if item.file_size > MAX_ENTRY_SIZE:
                        raise ValueError(
                            f"ZIP entry too large: {item.filename} "
                            f"({item.file_size} bytes, max {MAX_ENTRY_SIZE})"
                        )
